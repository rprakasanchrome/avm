...
void parity_hiding_trellis_off(const struct AV2_COMP *cpi, MACROBLOCK *mb,
  ...
  const qm_val_t *iqmatrix = av2_get_iqmatrix(&cpi->common.quant_params, xd,
                                              plane_type, tx_size, tx_type.prim_tx);
...
void av2_quant(const int use_tcq_deadzone_boost, MACROBLOCK *x, int plane,
  ...
  const struct macroblock_plane *const p = &x->plane[plane];
  const TX_TYPE tx_type = { .prim_tx = txfm_param->prim_tx_type,
                            .sec_tx = txfm_param->sec_tx_type,
                            .sec_set = txfm_param->sec_tx_set };
  const SCAN_ORDER *const scan_order =
      get_scan(txfm_param->tx_size, tx_type);
  const int block_offset = BLOCK_OFFSET(block);
  tran_low_t *const coeff = p->coeff + block_offset;
  tran_low_t *const qcoeff = p->qcoeff + block_offset;
  ...
  set_bob(x, plane, block, txfm_param->tx_size, tx_type);
...
void av2_setup_qmatrix(const CommonQuantParams *quant_params,
  ...
  qparam->qmatrix = av2_get_qmatrix(quant_params, xd, plane, tx_size, tx_type.prim_tx);
  qparam->iqmatrix =
      av2_get_iqmatrix(quant_params, xd, plane, tx_size, tx_type.prim_tx);
}
...
static void encode_block(int plane, int block, int blk_row, int blk_col,
  ...
  if (!is_blk_skip(x->txfm_search_info.blk_skip[plane],
    ...
    bool enable_parity_hiding =
        ...
        plane == PLANE_TYPE_Y &&
        ph_allowed_tx_types[tx_type.prim_tx] &&
    ...
    if (p->eobs[block] == 1 && plane == PLANE_TYPE_Y && !is_inter) {
      if (tx_type.prim_tx != DCT_DCT || (stx && primary_tx_type)) {
    ...
    if (p->eobs[block] <= 3 && plane == PLANE_TYPE_Y && is_inter && stx) {
      ...
      tx_type = make_tx_type(primary_tx_type);
    }
...
void av2_encode_block_intra(int plane, int block, int blk_row, int blk_col,
  ...
  if (plane == 0 && is_blk_skip(x->txfm_search_info.blk_skip[plane],
    ...
    *bob_code = 0;
    ...
    if (*eob == 1 && tx_type.prim_tx != DCT_DCT && plane == 0) {
