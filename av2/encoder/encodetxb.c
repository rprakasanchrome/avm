...
static AVM_FORCE_INLINE int warehouse_efficients_txb_laplacian(
  ...
  if (((cm->seq_params.enable_fsc &&
       ...
       use_inter_fsc(cm, plane, get_primary_tx_type(tx_type),
...
int av2_cost_coeffs_txb_estimate(const MACROBLOCK *x, const int plane,
  ...
  const SCAN_ORDER *scan_order = get_scan(tx_size, get_primary_tx_type(tx_type));
    ...
    if (eset > 0) {
      ...
      if (is_inter) {
        ...
        if (allow_update_cdf && cm->seq_params.enable_inter_ist &&
            block_signals_sec_tx_type(xd, tx_size, get_primary_tx_type(tx_type), eob)) {
