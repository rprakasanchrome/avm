...
//1. Primary 2D Transform Type (1 Byte Enum: 0...15)
enum {
...
} UENUM1BYTE(PRIM_TX_TYPE);

//2. Secondary Transform Type / Kernel (1 Byte Enum: 0...3)
typedef enum {
  SEC_TX_NONE,
  SEC_TX_KERNEL_1,
  SEC_TX_KERNEL_2,
  SEC_TX_KERNEL_3,
  SEC_TX_TYPES_COUNT,
} UENUM1BYTE(SEC_TX_TYPE);

//3. Secondary Transform Set Index (1 Byte Enum: 0...6)
enum {
  SEC_TX_SET_0,
  SEC_TX_SET_1,
  SEC_TX_SET_2,
...
//4. Unified Transform Union (2 Bytes Total)
typedef union {
  uint16_t packed_tx_type;  // Full 16-bit packed bitfield value
  struct {
    PRIM_TX_TYPE prim_tx : 4;      // Bits 0...3: Primary transform
    SEC_TX_TYPE sec_tx : 2;        // Bits 4..5: Secondary transform type
    SEC_TX_SET sec_set : 4;       // Bits 6..9: Secondary transform set index
  };
} TX_TYPE;

static INLINE TX_TYPE make_tx_type(PRIM_TX_TYPE prim_tx) {
  TX_TYPE tx;
  tx.packed_tx_type = 0;
  tx.prim_tx = prim_tx;
  return tx;
